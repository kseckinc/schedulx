...
ycDDowEZeqYBy2p+vAx1AZWP5TJ6/gYWGkbjp9YU7hfcEjcmWNF9+ca4/RNcye8yyW
...
QTaXePw5qxDKU1TTzEpC9LYE97m83Vg/jPttT/XL9zh+OzlEji3XEQgQM6cp
...
func gophNewTest(t *testing.T) {
	ip := "10.192.211.233"
	uname := "root"
	pwd := "123456"

	_, err := goph.NewUnknown(uname, ip, goph.Password(pwd))
	//_, err := goph.New(uname, ip, goph.Password(pwd))
	t.Log(err)
}
...
func newServer(port string) {
	...
	config := &ssh.ServerConfig{
		...
		PasswordCallback: func(c ssh.ConnMetadata, pass []byte) (*ssh.Permissions, error) {
			...
			if c.User() == "root" && string(pass) == "123456" {
				return nil, nil
			}
